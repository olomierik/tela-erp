...
export function generatePDFReport({
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.line(margin, ph - 18, pageWidth - margin, ph - 18);
    doc.setFontSize(8);
    doc.setTextColor(140, 140, 160);
    doc.setFont('helvetica', 'normal');
    doc.text(`${title} · Generated by TELA-ERP`, margin, ph - 11);
    doc.text(`Page ${i} of ${pageCount}`, pageWidth - margin, ph - 11, { align: 'right' });
    // Tela branding
    try { doc.addImage(TELA_LOGO_BASE64, 'PNG', margin, ph - 8, 18, 6); } catch { /* skip */ }
    doc.setFontSize(6.5);
    doc.setTextColor(120, 130, 155);
    doc.setFont('helvetica', 'normal');
    doc.text('Erick Elibariki Olomi  0752401012', margin + 21, ph - 4);
  }
...
export function generateInvoicePDF(opts: InvoicePDFOptions) {
  ...
  // Top row: company reference & generated date
  doc.setFontSize(7.5);
  doc.setTextColor(255, 255, 255);
  doc.setFont('helvetica', 'normal');
  doc.text(`${opts.tenantName || 'Company'} · Invoice ${opts.invoiceNumber}`, margin, ph - 12);
  doc.text(`Generated ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}`, pageWidth - margin, ph - 12, { align: 'right' });
  // Bottom row: Tela branding
  try { doc.addImage(TELA_LOGO_BASE64, 'PNG', margin, ph - 9, 18, 6); } catch { /* skip */ }
  doc.setFontSize(7);
